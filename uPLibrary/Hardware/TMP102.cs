    ...
    public class TMP102
    {
        ...
        public bool ExtendedMode
        {
            ...
            set
            {
                ...
                if (value)
                    throw new ApplicationException("Extendend Mode not supported yet!");
        ...
        private void ReadRegister(byte[] register, byte[] data)
        {
            ...
            // write address register to read
            i2cTx[0] = I2CDevice.CreateWriteTransaction(register);
            // read data from register
            i2cTx[1] = I2CDevice.CreateReadTransaction(data);

            // execute transaction
            if (this.i2c.Execute(i2cTx, TIMEOUT_TRANS) != (data.Length + register.Length))
        ...
        private void WriteRegister(byte[] register, byte[] data)
        {
            ...
            // write address register and data to write
            i2cTx[0] = I2CDevice.CreateWriteTransaction(new byte[] { register[0], data[0], data[1] });

            // execute transaction
            if (this.i2c.Execute(i2cTx, TIMEOUT_TRANS) != (data.Length + register.Length))
                throw new ApplicationException("Error executing I2C writing to register");
        }
    }
